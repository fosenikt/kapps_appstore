...
class Companies extends Db
{
	...
	public function get_company($id)
	{
		...
		if ($stmt = $db->prepare('SELECT * FROM company WHERE public_id=?')) {
	...
	public function get_companies()
	{
		...
		// Prepare our SQL, preparing the SQL statement will prevent SQL injection.
		if ($stmt = $db->prepare('SELECT * FROM company ORDER BY title ASC')) {
			// Bind parameters (s = string, i = int, b = blob, etc), in our case the username is a string so we use "s"
	...
	public function get_companies_simple()
	{
		...
		// Prepare our SQL, preparing the SQL statement will prevent SQL injection.
		if ($stmt = $db->prepare('SELECT id, title FROM company ORDER BY title ASC')) {
			// Bind parameters (s = string, i = int, b = blob, etc), in our case the username is a string so we use "s"
			//$stmt->bind_param('s', $mail);
			$stmt->execute();
		}
	...
	private function store_public_id($company_id, $public_id)
	{
		...
		// Prepare our SQL, preparing the SQL statement will prevent SQL injection.
		if ($stmt = $db->prepare('UPDATE company SET public_id=? WHERE id=?')) {
			// Bind parameters (s = string, i = int, b = blob, etc), in our case the username is a string so we use "s"
